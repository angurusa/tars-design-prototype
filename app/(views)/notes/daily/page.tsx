...
export default function DailyNotesPage() {
  ...
  const state = getPageState("/notes/daily");
  ...
  if (state === "empty") {
    return (
      <EmptyState
        ...
        description="The agent runs at 6pm on weekdays, or you can generate one now."
  ...
  function renderContent() {
    ...
    if (selected.status === "missing" || !selected.content) {
      return (
        <EmptyState
          icon={FileText}
          title={`No note for ${selected.displayLabel}`}
          description="This day has no generated note."
          action={{ label: "Generate Now", onClick: () => {} }}
        />
      );
    }
        ...
        {state === "streaming" && (
          <StatusBanner variant="info">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium">Generating daily note...</p>
            </div>
          </StatusBanner>
        )}
